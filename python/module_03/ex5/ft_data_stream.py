...
def consume_event(list_events: list[str, str]) -> typing.Generator[tuple[tuple[str, str], list[tuple[str, str]]], None, None]:

    while list_events:
        index = random.randrange(0, len(list_events))
        event = list_events[index]
        list_events = [list_events[x] for x in range(len(list_events)) if x != index]
