...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/09 14:51:51 by username          #+#    #+#             */
/*   Updated: 2026/04/09 16:03:30 by adrianag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
cost_b calculates the minimum number of moves needed to place a node at the
top of the stack B. Returns positive for rotations up (rb) and negative for
rotations down (rrb).
*/
...
int	cost_a(t_list *a, t_list *nod)
{
	...
	while (tmp)
	{
		if (prv->idx < nod->idx && nod->idx < tmp->idx)
			break ;
		if (prv->idx > tmp->idx && (nod->idx > prv->idx || nod->idx < tmp->idx))
			break ;
		cost_ra++;
		prv = tmp;
		tmp = tmp->next;
...
in the same direction. Therefore, we count the move as 1. If the sign of
the costs is the opposite, we sum both costs.
*/
